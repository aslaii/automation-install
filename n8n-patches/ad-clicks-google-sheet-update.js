...
const clicksBySku = new Map();
for (const row of reportData) {
  const sku = String(row.advertisedSku || '').trim();
  if (!sku) continue;
  const clicks = toNumber(row.clicks);
  clicksBySku.set(sku, Number(((clicksBySku.get(sku) || 0) + clicks).toFixed(2)));
}

const headerRow = values[0] || [];
const skuCol = headerRow.findIndex((cell) => normalizeHeader(cell) === 'SKU');
const targetCol = headerRow.findIndex((cell) => normalizeHeader(cell) === 'ADCLICKS');

if (skuCol === -1 || targetCol === -1) {
  throw new Error('Could not find SKU and AD_CLICKS columns');
}
...
for (let i = 1; i < values.length; i++) {
  ...
  data.push({
    ...
    values: [[Number((clicksBySku.get(sku) || 0).toFixed(2))]],
  });
}
