...
const DEFAULT_WORKFLOW = path.resolve(__dirname, '..', 'workflows', 'Get CTR.json');
const DEFAULT_REPORT = path.resolve(__dirname, 'tests', 'fixtures', 'ctr-report-contract.json');
const DEFAULT_SHEET = path.resolve(__dirname, 'tests', 'fixtures', 'ctr-workflow-local-sheet-workflow.json');
const DEFAULT_NODE_NAME = 'Code in JavaScript';
const DEFAULT_MISMATCH_PREVIEW_LIMIT = 10;
const TARGET_HEADER_ALIASES = ['CTR'];
...
function extractSheetContext(sheetFixture, fallbackSheetName = 'Sheet1') {
  ...
  if (headerRowIndex === -1) {
    throw new Error('source=sheet missing required SKU and CTR columns');
  }
...
function buildLocalBatchUpdate({ sheetContext, computation }) {
  ...
  const data = sheetContext.targets.map((target) => {
    ...
    const value = roundCtr(item?.ctr || 0);
    return {
      range: target.range,
      values: [[value]],
    };
  ...
  for (const range of ranges) {
    ...
    if (!local || !workflow || local.value !== workflow.value) {
      mismatchPreview.push({
        ...
        delta: local && workflow ? roundCtr(workflow.value - local.value) : null,
      });
    }
  }
...
function buildCtrCsvRow(row = {}, index = 0) {
  const products = String(row.Products || row.products || row.productLabel || row.product || row.label || '').trim();
  const clicks = row.Clicks ?? row.clicks;
  const impressions = row.Impressions ?? row.impressions;
  const ctr = row.CTR ?? row.ctr;
  const status = row.Status ?? row.status ?? '';

  if (!products) {
    throw new Error(`CTR workflow fixture fileRows row ${index + 1} is missing Products`);
  }
  ...
  for (const [label, value] of requiredPairs) {
    const parsed = Number(String(value ?? '').replace(/[\s,%]/g, ''));
    if (!Number.isFinite(parsed)) {
      throw new Error(`CTR workflow fixture fileRows row ${index + 1} has invalid ${label}`);
    }
  }

  return [products, status, clicks, impressions, ctr];
}

async function loadExpectedInput({ sheetFixture, date, tempDirPrefix = 'ctr-workflow-local-' }) {
  ...
  if (Array.isArray(sheetFixture?.fileRows)) {
    ...
    const tempPath = path.join(tmpDir, `ctr-file-input-${date}.csv`);
    const headers = ['Products', 'Status', 'Clicks', 'Impressions', 'CTR'];
    const lines = [headers.join(',')];

    sheetFixture.fileRows.forEach((row, index) => {
      lines.push(buildCtrCsvRow(row, index).map(escapeCsvValue).join(','));
    });

    await fs.promises.writeFile(tempPath, `${lines.join('\n')}\n`, 'utf8');
    try {
      return await loadCtrFromFile(tempPath, date);
    } finally {
  ...
  if (sheetFixture?.fileInputPath) {
    return loadCtrFromFile(path.resolve(sheetFixture.fileInputPath), date);
  }
...
async function runWorkflowLocally(options = {}) {
  ...
  let localUpdates = [];
  try {
    parsedReport = parseCtrReport(JSON.stringify(reportRows));
    expectedInput = await loadExpectedInput({ sheetFixture, date });
    localComputation = computeCtr({
      parsedReport,
      expectedInput,
      source: 'file',
      tolerance: 0.0001,
...
async function main(argv = []) {
  ...
  if (options.help) {
    console.log('Usage: node run-ctr-workflow-local.js [--json] [--workflow path] [--report path] [--sheet path] [--date YYYY-MM-DD] [--sheet-name name] [--mismatch-preview-limit N]');
    return null;
  }
